...
export const Menu = forwardRef(function MenuFR(
...
const HoveringToolbar: FC<Props> = ({ onEditHyperlink }) => {
  ...
  const above: CustomElement | undefined = Editor.above(
    editor
  )?.[0] as CustomElement;
  ...
  useEffect(() => {
    ...
    if (!selection || above?.type !== CustomElementType.link) {
  ...
  return (
    <Portal>
      <Menu ref={ref}>
        <a href={above?.url} target="_blank" rel="noreferrer">
          {above?.url}
        </a>
        <IconButton
          color="primary"
          ...
          component="label"
          onClick={() =>
            onEditHyperlink && onEditHyperlink(Editor.above(editor)?.[1])
          }
        >
          <EditIcon />
        </IconButton>
